...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ManagedChannel channel = ManagedChannelBuilder.forAddress(options.getSandboxHost(), options.getSandboxPort())
              .usePlaintext()
              .maxInboundMessageSize(Integer.MAX_VALUE)
              .build();
      runBots(client, channel);
  ...
  public static void runBots(DamlLedgerClient client, ManagedChannel channel) {
    ...
    TimeManager timeManager = getTimeClientBasedTimeManager();

    AuctionFinalizeBot auctionFinalizeBot =
            new AuctionFinalizeBot(timeManager, APPLICATION_ID, parties.getAuctionAgent());

    CommissionBot commissionBot = new CommissionBot(timeManager, APPLICATION_ID, parties.getIssuer());
    ...
    MarketSetupSignerBot signerIssuer = marketSetupStarterBot.addNextSignerBot(parties.getIssuer());
    MarketSetupSignerBot signerCsd = marketSetupStarterBot.addNextSignerBot(parties.getCSD());
    MarketSetupSignerBot signerAuctionA = marketSetupStarterBot.addNextSignerBot(parties.getAuctionAgent());
    MarketSetupSignerBot signerOp = marketSetupStarterBot.addNextSignerBot(parties.getOperator());
    MarketSetupSignerBot signerReg = marketSetupStarterBot.addNextSignerBot(parties.getRegulator());
    MarketSetupSignerBot signerCentral = marketSetupStarterBot.addNextSignerBot(parties.getCentralBank());
